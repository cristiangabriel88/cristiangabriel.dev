...
function PixelGrid({ rows, scale = 3, color, style = {}, className = '' }) {
  const h = rows.length;
...
function PixelIcon({ name, scale = 3 }) {
  const rows = TECH_ICONS[name];
  if (!rows) return null;
  return React.createElement(PixelGrid, { rows, scale });
}
...
function PixelDivider({ width = 520 }) {
  const segments = [];
  const itemPx = 28;
  ...
  for (let i = 0; i < count; i++) {
    const variant = i % 5;
    let rows;
    if (variant === 0) rows = SPRITES.tinyLeaf;
    else if (variant === 1) rows = ['  ##  ', '  ##  '];
    else if (variant === 2) rows = SPRITES.tinyLeaf;
    else if (variant === 3) rows = ['  ##  ', '  ##  '];
    else rows = SPRITES.sprout;
    segments.push(
      React.createElement('div', { key: i, style: { display: 'flex', alignItems: 'flex-end' } },
        React.createElement(PixelGrid, { rows, scale: 2 })
      )
    );
  }
  return React.createElement('div', { className: 'pixel-divider' },
    React.createElement('span', { className: 'line' }),
    React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'flex-end' } }, segments),
    React.createElement('span', { className: 'line' }),
  );
}
...
Object.assign(window, { PixelGrid, PixelIcon, PixelDivider, TECH_ICONS, SPRITES, CHARACTER_FRAMES });
