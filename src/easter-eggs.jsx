  ...
  function Terminal({ open, onClose, autoLaunchGame = false }) {
    ...
    function updateGhost() {
      ...
      if (val && el.selectionStart === val.length && el.selectionEnd === val.length) {
        ...
        const res =
          gameMode === "forest" && F && F.complete
            ? F.complete(val, gameStateRef.current)
            : completeCommand(val);
    ...
    const TERM_COMMANDS = [
      "about", "ascii", "clear", "contact", "date", "exit",
      "forest", "help", "projects", "snake", "stack", "whoami",
    ];
    function completeCommand(val) {
      const endsSpace = /\s$/.test(val);
      const words = val.trim() ? val.trim().split(/\s+/) : [];
      if (words.length !== 1 || endsSpace) return { completion: null, candidates: [] };
      const prefix = words[0].toLowerCase();
      const matches = TERM_COMMANDS.filter((c) => c.indexOf(prefix) === 0);
      if (!matches.length) return { completion: null, candidates: [] };
      ...
      return { completion: common.length > prefix.length ? common : null, candidates: matches };
    }
    ...
    function forestMenuItems(F) {
      ...
      const items = [
        { id: "new", label: "new game", aliases: ["new", "new game"] },
      ];
      if (hasSave) items.push({ id: "resume", label: "resume your walk", aliases: ["resume"] });
      items.push({ id: "endings", label: "endings & achievements", aliases: ["endings", "achievements"] });
      items.push({ id: "help", label: "how to play", aliases: ["help", "how to play"] });
      items.push({ id: "seed", label: "new game with a seed", aliases: ["seed"] });
      return items;
    }
    ...
    function forestMenuLines(F) {
      const out = [
        { text: "", kind: "game" },
        { text: "F O R E S T   W A N D E R E R", kind: "room" },
        { text: "a small adventure in the woods.", kind: "dim" },
        { text: "", kind: "game" },
      ];
      forestMenuItems(F).forEach((it, i) => {
        out.push({ text: "  " + (i + 1) + ") " + it.label, kind: "choice" });
      });
      out.push({ text: "", kind: "game" });
      out.push({ text: "(type a number.)", kind: "dim" });
      return out;
    }
    ...
    function resumeForestGame(F) {
      ...
      enqueueGameLines(
        [{ text: "(resumed.)", kind: "dim" }].concat(ok.lines),
      );
    }
    ...
    function onInputKeyDown(e) {
      ...
      if (e.key === "Tab") {
        ...
        const res =
          gameMode === "forest" && F && F.complete
            ? F.complete(el.value, gameStateRef.current)
            : completeCommand(el.value);
        if (res && res.completion) {
          setInputValue(res.completion);
        } else if (res && res.candidates && res.candidates.length > 1) {
          enqueueGameLines([{ text: res.candidates.join("   "), kind: "dim" }]);
        }
        return;
      }
    ...
    function onSubmit(e) {
      ...
      if (gameMode === "forest-menu") {
        ...
        if (!choice) {
          ...
          enqueueGameLines(
            [{ text: "that's not one of the options.", kind: "dim" }]
      ...
      if (gameMode === "forest-seed") {
        ...
        if (lower === "cancel" || lower === "back") {
          ...
          startForestGame(F, { seed: parseInt(trimmed, 10) });
        } else {
